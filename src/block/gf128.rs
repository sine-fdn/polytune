...
mod scalar {
    ...
    pub fn clmul64(x: u64, y: u64) -> u128 {
        ...
        // 25 multiplications in this method. However, the bearssl version uses 64 bit multiplications instead of
        // 128 bit mults.
        const HOLES: u64 = 0b0001000010000100001000010000100001000010000100001000010000100001;
        const HOLES_U128: u128 = const { (HOLES as u128) << 65 | HOLES as u128 };
        let x0 = (x & const { HOLES }) as u128;
        let x1 = (x & const { HOLES << 1 }) as u128;
        let x2 = (x & const { HOLES << 2 }) as u128;
        let x3 = (x & const { HOLES << 3 }) as u128;
        let x4 = (x & const { HOLES << 4 }) as u128;
        let y0 = (y & const { HOLES }) as u128;
        let y1 = (y & const { HOLES << 1 }) as u128;
        let y2 = (y & const { HOLES << 2 }) as u128;
        let y3 = (y & const { HOLES << 3 }) as u128;
        let y4 = (y & const { HOLES << 4 }) as u128;
        ...
        z0 &= const { HOLES_U128 };
        z1 &= const { HOLES_U128 << 1 };
        z2 &= const { HOLES_U128 << 2 };
        z3 &= const { HOLES_U128 << 3 };
        z4 &= const { HOLES_U128 << 4 };
...
mod scalar_simd_tests {
    ...
    #[test]
    fn test_clmul64() {
        let (a, b) = (u64::MAX, u64::MAX);
        unsafe {
            let clmul_res =
                _mm_clmulepi64_si128::<0x00>(transmute(a as u128), transmute(b as u128));
            let scalar_res = scalar::clmul64(a, b);
            assert_eq!(
                scalar_res,
                transmute(clmul_res),
                "differing clmul64 bits. input: {a}, {b}"
            );
        }
    }
    ...
    fn test_clmul128() {
        for i in 0..10000 {
            ...
            unsafe {
                ...
                assert_eq!(
                    scalar_res.0,
                    transmute(clmul_res.0),
                    "differing low bits. input: {a}, {b}"
                );
                assert_eq!(
                    scalar_res.1,
                    transmute(clmul_res.1),
                    "differing high bits. input: {a}, {b}"
                );
            }
        }
    }
        ...
        for i in 0..1000 {
            ...
            unsafe {
                ...
                assert_eq!(
                    scalar_res,
                    transmute(clmul_res),
                    "differing gf128_reduce bits. input: {a}, {b}"
                );
            }
        }
    }
        ...
        for i in 0..1000 {
            ...
            unsafe {
                ...
                assert_eq!(
                    scalar_res,
                    transmute(clmul_res),
                    "differing gf128_mulq bits. input: {a}, {b}"
                );
            }
        }
    }
