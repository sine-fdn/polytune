...
let fhaand_compute_hashes
    ...
    : Prims.Pure
      ...
      (requires
        b2t
        (((Alloc.Vec.impl_1__len #bool #Alloc.Alloc.t_Global yi <: usize) >=. l <: bool) &&
          ((Core.Slice.impl__len #bool s <: usize) >=. n <: bool)) /\
...
let fhaand_compute_vi
      (i n l: usize)
      (xshares: t_Slice Polytune.Data_types.t_Share)
      (vi: t_Slice bool)
      (h0h1_j:
          Alloc.Vec.t_Vec (Alloc.Vec.t_Vec (bool & bool) Alloc.Alloc.t_Global) Alloc.Alloc.t_Global)
    : Prims.Pure (t_Slice bool)
      (requires
        b2t ((Core.Slice.impl__len #bool vi <: usize) =. l <: bool) /\
        b2t ((Core.Slice.impl__len #Polytune.Data_types.t_Share xshares <: usize) >=. l <: bool) /\
        (forall (ll: usize).
            b2t
            ((mk_usize 0 <=. ll <: bool) && (ll <. l <: bool) &&
  ...
  Rust_primitives.Hax.Folds.fold_range (mk_usize 0)
    n
    (fun vi temp_1_ ->
        let vi:t_Slice bool = vi in
        let _:usize = temp_1_ in
        (Core.Slice.impl__len #bool vi <: usize) =. l <: bool)
    vi
    (fun vi j ->
        let vi:t_Slice bool = vi in
        let j:usize = j in
